...
import com.sm.onlinegamestore.api.StoreService
...
class StoreLoader extends LagomApplicationLoader {
  ...
  override def describeService: Option[Descriptor] = Some(readDescriptor[StoreService])
}
...
abstract class StoreApplication(context: LagomApplicationContext)
  ...
  readSide.register(new StoreReportProcessor())

  // Bind the service that this server provides
  override lazy val lagomServer: LagomServer = serverFor[StoreService](wire[StoreServiceImpl])
  ...
  clusterSharding.init(
    Entity(Store.typeKey)(
      entityContext => Store.apply(entityContext)
    )
  )
